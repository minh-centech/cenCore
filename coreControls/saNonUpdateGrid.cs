...
namespace coreControls
{
    public partial class saNonUpdateGrid : UltraGrid
    {
            ...
            if (CustomizeColumns == 0)
            {
                ...
                if (e.Layout.Bands.Count > 0)
                {
                    ...
                    foreach (Infragistics.Win.UltraWinGrid.UltraGridColumn ugcol in saBand.Columns)
                    {
                        ...
                        //if ((ugcol.Key.StartsWith("ID") | HiddenColumnList.IndexOf("("+ugcol.Key.ToUpper()+")")>=0) & DropdownColumnList.IndexOf("("+ugcol.Key.ToUpper()+")")<0)
                        //if ((ugcol.Key=="ID" | HiddenColumnList.IndexOf("(" + ugcol.Key.ToUpper() + ")") >= 0) | (ugcol.Key.StartsWith("ID") & isReport))
                        ...
                        if (!coreCommon.coreCommon.IsNull(HiddenColumnsList) && HiddenColumnsList.ToUpper().IndexOf("[" + ColumnKey + "]") >= 0)
                        {
                            ugcol.Hidden = true;
                        }
                        ...
                        if (!coreCommon.coreCommon.IsNull(FixedColumnsList) && FixedColumnsList.ToUpper().IndexOf("[" + ColumnKey + "]") >= 0)
                        {
                    ...
                    if (AddCheckBoxColumn && saBand.Columns.Count > 0)
                    {
                        ...
                        saBand.Columns["CheckColumn"].Header.Fixed = true;
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            {
                ActiveRow.Cells["CheckColumn"].Value = !coreCommon.coreCommon.stringParseBoolean(ActiveRow.Cells["CheckColumn"].Value);
                UpdateData();
            }
            else
                base.OnKeyDown(e);
